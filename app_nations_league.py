...
st.set_page_config(page_title="Betting Assistant - Nations League Backup", page_icon="🏆")
st.title("Betting Assistant")
st.caption("⚠️ BACKUP-VERSION: UEFA Nations League (ikke Big 5-ligaerne)")
...
def _byg_forslag_for_kamp(kamp, liga, marked):
    ...
    if marked == "Over/Under 2.5":
        if liga == "UEFA Nations League":
            # NATIONS LEAGUE-BACKUP: football-data.org har ingen adgang til
            # denne turnering (bekraeftet empirisk, se
            # test_nations_league_adgang.py), saa der er intet datagrundlag
            # for hent_team_id()/hent_historisk_over_rate(). Bevidst,
            # dokumenteret begraensning for backup-grenen.
            risikobillede["value_signal"] = {
                "status": "INSUFFICIENT_DATA",
                "forskel_pp": None,
                "n_hjemme": 0,
                "n_ude": 0,
                "turnering_hjemme": None,
                "turnering_ude": None,
                "liga": liga,
            }
        else:
...
if st.session_state.afklaring:
    ...
    if not st.session_state.messages:
        st.chat_message("assistant").markdown(
            tilfoej_ikoner(
                "Hej! Dette er en midlertidig backup-version, der udelukkende "
                "giver forslag til UEFA Nations League. Skal vi finde et "
                "forslag til dig?"
            )
        )
